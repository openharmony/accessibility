 ...
 */
HWTEST_F(AAMSServerTest, CalculateClickPosition_PartialOverlap_001, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "AAMS_moduletest_CalculateClickPosition_PartialOverlap_001 start";
    auto &aams = Singleton<AccessibleAbilityManagerService>::GetInstance();
    auto accountData = aams.GetCurrentAccountData();
    ...
    // Element rect partially outside window bounds (window: 0,0 to 1000,1000, element: -50,-50 to 50,50)
    Rect rect;
    rect.SetLeftTopScreenPostion(-50, -50);
    rect.SetRightBottomScreenPostion(50, 50);
    ...
    EXPECT_TRUE(result);
    EXPECT_EQ(xPos, 0); // (0 + 0) / 2, clipped to window bounds
    EXPECT_EQ(yPos, 0); // (0 + 0) / 2, clipped to window bounds

    aams.DeregisterElementOperatorByWindowId(0, 0);
    GTEST_LOG_(INFO) << "AAMS_moduletest_CalculateClickPosition_PartialOverlap_001 end";
}

/**
 * @tc.number: AAMS_moduletest_CalculateClickPosition_OutOfBounds_001
 * @tc.name: CalculateClickPosition with element completely outside window bounds
 * @tc.desc: Test CalculateClickPosition when element is completely outside window bounds.
 */
HWTEST_F(AAMSServerTest, CalculateClickPosition_OutOfBounds_001, TestSize.Level1)
{
    GTEST_LOG_(INFO) << "AAMS_moduletest_CalculateClickPosition_OutOfBounds_001 start";
    auto &aams = Singleton<AccessibleAbilityManagerService>::GetInstance();
    auto accountData = aams.GetCurrentAccountData();
    ...
    // Element rect completely outside window bounds (window: 0,0 to 1000,1000, element: -100,-100 to -50,-50)
    Rect rect;
