    ...
    int32_t *errorCode)
{
    ...
    if (Utils::CheckAbilityType(abilityTypeStr)) {
        abilityTypes = OHOS::AccessibilityNapi::ConvertStringToAccessibilityAbilityTypes(abilityTypeStr);
    } else {
        *errorCode = ERR_INPUT_INVALID;
    ...
    if (Utils::CheckStateType(stateTypeStr)) {
        stateTypes = OHOS::AccessibilityNapi::ConvertStringToAbilityStateType(stateTypeStr);
    } else {
