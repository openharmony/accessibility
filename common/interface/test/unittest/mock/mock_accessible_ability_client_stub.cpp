...
#define SWITCH_END() default:\
    {\
        ...
        HILOG_WARN("AccessibleAbilityClientStubMock::OnRemoteRequest, default case, need check.");\
        break;\
    }\
}
...
AccessibleAbilityClientStubMock::AccessibleAbilityClientStubMock()
{
}
...
int AccessibleAbilityClientStubMock::OnRemoteRequest(uint32_t code,
    MessageParcel &data, MessageParcel &reply, MessageOption &option)
{
    HILOG_DEBUG("AccessibleAbilityClientStubMock::OnRemoteRequest, cmd = %d, flags= %d", code, option.GetFlags());
    std::u16string descriptor = AccessibleAbilityClientStubMock::GetDescriptor();
...
ErrCode AccessibleAbilityClientStubMock::HandleInit(MessageParcel &data, MessageParcel &reply)
{
    HILOG_DEBUG();
...
ErrCode AccessibleAbilityClientStubMock::HandleDisconnect(MessageParcel &data, MessageParcel &reply)
{
    HILOG_DEBUG();
    int32_t channelId = data.ReadInt32();
...
ErrCode AccessibleAbilityClientStubMock::HandleOnAccessibilityEvent(MessageParcel &data, MessageParcel &reply)
{
    HILOG_DEBUG();
...
ErrCode AccessibleAbilityClientStubMock::HandleOnKeyPressEvent(MessageParcel &data, MessageParcel &reply)
{
    HILOG_DEBUG();
