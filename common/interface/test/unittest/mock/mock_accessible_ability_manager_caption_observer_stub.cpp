...
AccessibleAbilityManagerCaptionObserverStubMock::AccessibleAbilityManagerCaptionObserverStubMock()
{
    HILOG_DEBUG();
}

int AccessibleAbilityManagerCaptionObserverStubMock::OnRemoteRequest(uint32_t code, MessageParcel &data,
    MessageParcel &reply, MessageOption &option)
{
    HILOG_DEBUG("cmd = %{public}d, flags = %{public}d", code, option.GetFlags());
    std::u16string descriptor = AccessibleAbilityManagerCaptionObserverStubMock::GetDescriptor();
...
ErrCode AccessibleAbilityManagerCaptionObserverStubMock::HandleOnCaptionPropertyChanged(MessageParcel &data,
    MessageParcel &reply)
{
    HILOG_DEBUG();
