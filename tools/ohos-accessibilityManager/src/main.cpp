...
void PrintHelp(const std::string& subcommand = "")
{
    if (subcommand.empty()) {
        ...
        std::cout << "  --token-id  <value>     Caller's access token ID for permission usage recording" << std::endl;
        std::cout << std::endl;
...
int CommandAbilityEnable(const std::vector<std::string>& args)
{
    ...
    if (ret == RetError::RET_OK) {
        Security::AccessToken::PrivacyKit::AddPermissionUsedRecord(g_callerTokenId,
...
int main(int argc, char* argv[])
{
    ...
    std::string tokenIdStr = GetOption(args, "--token-id");
    if (!tokenIdStr.empty()) {
        int32_t tokenIdValue = 0;
        if (ConvertStringToInt(tokenIdStr, tokenIdValue) && tokenIdValue > 0) {
            g_callerTokenId = static_cast<uint32_t>(tokenIdValue);
        }
    }
    if (g_callerTokenId == 0) {
        g_callerTokenId = static_cast<uint32_t>(GetSelfTokenID());
    }
